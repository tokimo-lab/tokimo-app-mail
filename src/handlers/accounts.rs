...
/// GET /api/apps/mail/providers — list all known provider presets.
...
/// GET /api/apps/mail/accounts — list all mail accounts for the current user.
pub async fn list_accounts(
    ...
    let accounts = services::accounts::list_accounts(&state.db, uid).await?;
    Ok(ok(accounts))
}

/// POST /api/apps/mail/accounts — create a new mail account.
pub async fn create_account(
    ...
    let uid: Uuid = user
        ...
        .parse()
        .map_err(|_| AppError::BadRequest("invalid user id".into()))?;
    let account = services::accounts::create_account(&state.db, uid, body).await?;
    Ok(ok(account))
}

/// GET /api/apps/mail/accounts/:id
pub async fn get_account(
    ...
    let account = services::accounts::get_account(&state.db, uid, account_id).await?;
    Ok(ok(account))
}

/// PATCH /api/apps/mail/accounts/:id
...
/// DELETE /api/apps/mail/accounts/:id
pub async fn delete_account(
    ...
    let account_id: Uuid = id
        .parse()
        .map_err(|_| AppError::BadRequest("invalid account id".into()))?;
    services::accounts::delete_account(&state.db, uid, account_id).await?;
    Ok(ok(()))
}

/// POST /api/apps/mail/accounts/:id/test — test IMAP/SMTP connection.
