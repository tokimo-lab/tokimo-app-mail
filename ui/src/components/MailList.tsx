...
export function MailList({
  ...
  const { data, isFetching, isLoading } = mailApi.listMessages.useQuery(
  ...
  const { data: searchData, isFetching: isSearchFetching } =
    mailApi.searchMessages.useQuery(
  ...
  const markReadMutation = mailApi.markRead.useMutation();
    ...
    (id: string) => {
      ...
      markReadMutation.mutate(
        { message_ids: [id] },
        {
          onSuccess: () => {
            if (!wasUnread) return;
            const key = mailApi.listFolders.queryKey({ accountId });
