...
export function MailViewer({
  ...
  const { data, isLoading } = mailApi.getMessage.useQuery(
    { messageId },
    { enabled: !!messageId },
  );
  const message = data as MailMessageFullOutput | undefined;

  const { mutate: markReadMutate } = mailApi.markRead.useMutation();
  const refetchBody = mailApi.refetchBody.useMutation({
    onSuccess: () => {
      // Invalidate the message query to re-render with new body.
      queryClient.invalidateQueries({
        queryKey: mailApi.getMessage.queryKey({ messageId }),
      });
    },
  });
